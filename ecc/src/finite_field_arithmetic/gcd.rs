use crate::finite_field_arithmetic::BigInt8;
...
impl LehmerMatrix {
    ...
    fn MixedApproximation(A: &BigInt8, B: &BigInt8) -> Self {
        ...
        let (mut hat_hat_A, mut hat_hat_B) = (
            BigInt8::new(&A.data[two_word_idx..].to_vec(), A.sign, b),
            if two_word_idx > nb - 1 {
                BigInt8::default()
            } else {
                BigInt8::new(&B.data[two_word_idx..].to_vec(), B.sign, b)
            },
        );
...
mod tests {
    ...
    fn test_not_coprime_gcd() {
        let (u, v, d, _) = BigInt8::euclid_extended_gcd(54, 189);
        // println!("---- u = {}, v = {}", u, v);
        assert_eq!(d, 27);
    }
    ...
    #[test]
    fn test_coprime_gcd() {
        let (u, v, d, _) = BigInt8::euclid_extended_gcd(45, 127);
        // println!("---- u = {}, v = {}", u, v);
        assert_eq!(d, 1);
    }
