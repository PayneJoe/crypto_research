...
mod tests {
    ...
    fn test_from() {
        ...
        assert_eq!(
            TestBigInt::from_str(R).unwrap().to_u64_digits().1,
            BigInteger::from(R).data
        );
        assert_eq!(
            TestBigInt::from_str(R2).unwrap().to_u64_digits().1,
            BigInteger::from(R2).data
        );
        assert_eq!(
            TestBigInt::from_str(R3).unwrap().to_u64_digits().1,
            BigInteger::from(R3).data
        );
        assert_eq!(
